...
.sc-nextstep-btn {
    border: 1px solid var(--color-header-grad-a, #0b6bcb);
    color: var(--color-header-grad-a, #0b6bcb);
    background: transparent;
    border-radius: var(--radius-small);
    padding: 3px 9px;
    font: inherit;
...
.sc-nextstep-btn:disabled {
    opacity: .45;
    cursor: not-allowed;
}

.sc-nextstep-btn:hover:not(:disabled) {
    background: linear-gradient(135deg, var(--color-header-grad-a), var(--color-header-grad-b));
    color: white;
}
...
/* Forward-to-agent picker: centered modal overlay, so the agent list can
   never be clipped by the chat's scroll container. */
.sc-forward-backdrop {
    position: fixed;
    inset: 0;
    z-index: 2000;
...
.sc-forward-picker-cancel:hover {
    background: var(--color-accent-soft);
    border-color: var(--color-accent);
    color: var(--color-accent-strong);
}
...
<header class="sc-header">
    ...
    <div class="sc-header-actions">
        ...
        <button
            ...
            class="sc-header-btn"
            id="sc-pipeline"
            title="Run the whole pipeline (Planner -> Engineer -> Builder) on your idea and try to produce a saved module file">
            <span>➤➤</span>
        ...
        <div class="sc-composer">
            ...
            <div class="sc-composer-inner">
                ...
                <div class="sc-composer-footer">
                    ...
                    <span class="sc-nextstep" title="Manually forward this agent's last reply to the next pipeline step">
                        <button
                            type="button"
                            class="sc-nextstep-btn"
                            id="sc-next-step"
                            disabled>
                            → Send to next step
                        </button>
                        <span class="sc-nextstep-name" id="sc-next-step-name"></span>
                    </span>
                </div>
...
/* Pipeline feature state (config/pipeline.json via /api/pipeline): the ordered
   step chain, and the most recent assistant reply the current agent produced
   (needed to forward it to the next step). */
let pipelineSteps = [];
...
/* Bubbles of the SAVED chat being viewed (openChat() renders it read-only).
   Used as the fallback source for the "Forward whole chat" transcript while
   a live session's own messages win whenever they are present. */
let openedChatMessages = [];

/* Chat ids for which the user already declined AI consolidation, so a
   read-only reopen does not nag about it again within this page session. */
const consolidationDismissed = new Set();
...
function buildBubbleActions(rawContent, bodyEl) {
    ...
    /* 4. Forward the WHOLE chat's output to another agent */
    const fwdChatBtn = el("button", "sc-bubble-btn", "📤");
    fwdChatBtn.type = "button";
    fwdChatBtn.title = "Forward whole chat to another agent";
    fwdChatBtn.setAttribute("aria-label", "Forward whole chat");
    fwdChatBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        toggleForwardPicker(null, bodyEl, fwdChatBtn, { wholeChat: true });
    });
    row.appendChild(fwdChatBtn);

    /* 5. Bookmark (placeholder) */
...
/* Make sure the agent catalog is populated (refetch once if boot missed it),
   so the forward picker ALWAYS has agents to list. */
async function ensureAgentsLoaded() {
    ...
    try {
        ...
        /* The modal shows "No other agents loaded." when the fetch fails. */
    }
}


/* Open/close the forward-to-agent dialog. Rendered as a CENTERED MODAL on
   document.body so it can never be clipped by the chat's scroll container or
   hidden behind a bubble - the agent list is always visible and clickable.
   wholeChat: true forwards the WHOLE conversation transcript instead of the
   single bubble's text. */
async function toggleForwardPicker(rawContent, bodyEl, fwdBtn, { wholeChat = false } = {}) {

    /* Close any existing picker overlay first. */
    ...
    document.querySelectorAll(".sc-bubble-btn.active")
    ...
    /* Backdrop: click anywhere outside closes the dialog. */
    const backdrop = document.createElement("div");
    backdrop.className = "sc-forward-backdrop";
    ...
    picker.setAttribute(
        "aria-label",
        wholeChat ? "Forward chat to" : "Forward to agent"
    );
    ...
    if (wholeChat) {
        picker.appendChild(
            el(
                "div",
                "sc-forward-picker-hint",
                "Sends the whole chat output into the other agent's input."
            )
        );
    }

    /* Every agent EXCEPT the one currently being chatted with, so you cannot
       forward back into its own conversation. */
    const agents = Array
    ...
    list.className = "sc-forward-picker-list";
    ...
    const cancel = el("button", "sc-forward-picker-cancel", "Cancel");
    cancel.type = "button";
...
/* Plain-text transcript of the CURRENT chat (live session when active, the
   opened saved chat otherwise): title + author:content lines per bubble. */
function buildChatTranscriptText() {
    ...
    ) ? activeSession.messages : openedChatMessages;
    ...
    const lines = [
        `# ${getChatTitle() || "Chat"}`,
        `(forwarded from ${activeAgent?.name || "the AI chat"})`,
        ""
    ];
    ...
    (messages || []).forEach(message => {
        ...
        lines.push(
            `**${author}**: ${String(message.text ?? message.content ?? "").trim()}`
        );
        lines.push("");
    });
...
function forwardChat(targetAgent) {

    if (!targetAgent) return;

    const transcript = buildChatTranscriptText();

    if (!transcript) {
        showToast("Nothing to forward - the chat is empty.");
        return;
    }

    /* Same delivery as per-message forwarding: open the target agent's chat
       page with the whole transcript pre-filled, ready to send. */
    const url = `/static/chat.html?agent=${encodeURIComponent(targetAgent.id)}&prefill=${encodeURIComponent(transcript)}`;
    window.open(url, `ai-chat-${targetAgent.id}`, "width=900,height=680,resizable=yes,scrollbars=yes");

    showToast(`Forwarding chat to ${targetAgent.name}…`);
}
...
async function openChat(id) {
    ...
    try {
        ...
        openedChatMessages = chat.messages || [];
        ...
        /* A 3+ version chat opened read-only can also be consolidated (unless
           the user already declined it during this page session). */
        if (
            Number(chat.version) >= 3 &&
            chat.version !== "C"
        ) {
            await maybeOfferConsolidation(chat);
        }
...
/* ============================================================
    13b. PIPELINE: TEST RUN + ONE-STEP-AHEAD FORWARDING
   ============================================================
   Two ways to use the Planner -> Engineer -> Builder chain:
     - Run Pipeline (header button): the server runs every step on one idea
       and returns each step's output + the final reply in a single request.
     - Send to next step (composer footer chip): manually forwards this
       agent's last reply into the chat of the NEXT step, so you can watch and
       learn how each stage behaves step by step.
   ============================================================ */

function updateNextStepChip() {

    if (!DOM.nextStep || !DOM.nextStepName) return;
...
async function runPipelineTest() {
    ...
    const idea =
        prompt(
            "Idea to run through the whole pipeline (Planner -> Engineer -> Builder):",
            context || "A Hello World box module"
        );
    ...
    try {
        ...
        addBubble(
            "agent",
            result.reply ||
                "(pipeline produced no final reply)",
            "Pipeline Result"
        );

        setSaveStatus(
            `Pipeline finished - ${outputs.length} step(s)`,
            "ok"
        );
...
async function forwardToNextStep() {
    ...
    const text =
        `[Previous step output from ${activeAgent.name}]\n\n` +
        lastAssistantReply;
...
/* Resurrect the server-side active chat after a page reload: the in-progress
   conversation lives on the server, so a refresh no longer finalizes it.
   Paints the restored bubbles, re-links the session id, and (when it is time)
   re-offers consolidation for a chat that already reached 3 versions. */
async function tryResumeActiveChat() {
    ...
    try {
        ...
        const query =
            activeAgent && activeAgent.id
                ? `?agent_id=${encodeURIComponent(activeAgent.id)}`
                : "";

        const response =
            await fetch(
                `/api/chats/active${query}`
            );
    ...
    activeSession.sessionId =
        chat.active === true
            ? (chat.id || activeSession.sessionId)
            : null;
    ...
    setSaveStatus(
        "Resumed the previous chat.",
        "ok"
    );

    /* A resumed open chat may already sit at 3+ versions - offer consolidation
       once (respecting any earlier decline within this page session). */
    if (
        Number(chat.version) >= 3 &&
        chat.version !== "C"
    ) {
...
/* Start the current agent completely fresh: finalize the active chat on the
   ...
   Reset asks FIRST: "save this chat as a new version?" - OK saves (the reset
   then becomes the NEXT version, exactly like an explicit Save), Cancel
   discards the in-progress chat without writing a transcript. Abort keeps the
...
async function startNewChat() {
    ...
    if (
        ...
        const proceed =
            window.confirm(
                "Start a new chat? This ends the current chat.\n\n" +
                "OK = continue, Cancel = stay in the current chat."
            );
        ...
        const save = window.confirm(
            "Save the current chat as a new version before starting fresh?\n\n" +
            "OK = save it, Cancel = discard it (no transcript is written)."
        );

        if (save) {
            await finalizeActiveChat({ quiet: true });
        } else {
            await discardActiveChat();
        }
    } else {
        await finalizeActiveChat({ quiet: true });
    }
...
async function refreshChatsDropdown() {
    ...
    chats.forEach(chat => {
        ...
        const tag =
            chat.status === "active"
                ? " (active)"
                : String(chat.version || "").toUpperCase() === "C"
                    ? " (consolidated)"
                    : chat.version
                        ? ` (v${chat.version})`
                        : "";
...
async function finalizeActiveChat({ quiet = false } = {}) {
    ...
    try {
        ...
        if (!quiet && result.consolidation_offered) {
            await maybeOfferConsolidation(result);
        }
...
/* Discard the active chat WITHOUT writing a transcript: the server drops the
   in-progress session entirely (no '# VERSION' section is appended). Used by
   the reset flow when the user chooses not to save. */
async function discardActiveChat() {
    ...
    try {
        ...
        setSaveStatus(
            "Previous chat discarded (no transcript written).",
            "ok"
        );
...
/* Offer AI consolidation for the chat that was just saved: once a transcript
   holds 3+ '# VERSION' sections the slot is worth turning into a summary +
   full-text archive (version "C"). Purely opt-in - Declining does nothing. */
async function maybeOfferConsolidation(chat) {

    const chatId =
        chat && (chat.id || chatIdFromResult(chat));

    if (!chatId || consolidationDismissed.has(chatId)) {
        return;
    }
    ...
    const consolidate =
        window.confirm(
            ...
            "single summary + full-conversation archive?\n\n" +
            "OK = run AI consolidation, Cancel = keep the versions as-is."
        );

    if (!consolidate) {
        consolidationDismissed.add(chatId);
        return;
    }
    ...
    try {
        ...
        if (!response.ok || !result.ok) {
            ...
            throw new Error(
                String(result.detail || result.error || "") ||
                "Consolidation failed."
            );
        }

        setSaveStatus(
            `Consolidated: ${result.summary_preview || ""}`.trim(),
            "ok"
        );
...
/* The /api/chats/end response carries id when the chat is saved; the legacy
   path only returned file/version, so recover the id from the file stem. */
function chatIdFromResult(chat) {

    if (!chat || !chat.file || chat.id) {
        return chat && chat.id;
    }
...
async function boot() {
    ...
    /* Reload resume: the server keeps the active chat (its messages survive a
       page refresh). Restore it so the conversation picks up where it was.
       Saved-test-run mode (?runTests=1) starts clean instead. */
    if (params.get("runTests") !== "1") {
        await tryResumeActiveChat();
    }
...
DOM.pipeline.addEventListener(
    "click",
    runPipelineTest
);


DOM.nextStep.addEventListener(
    "click",
    forwardToNextStep
);
...
/* ---------------------------------------------------------------------
   logs.html "Tool Usage" inspector feed
   --------------------------------------------------------------------- */
...
.tf-chip-ok { color: #3fb950; background: rgba(63, 185, 80, 0.12); }
.tf-chip-op-fail { color: #f85149; background: rgba(248, 81, 73, 0.14); }
.tf-chip-err { color: #f85149; background: rgba(248, 81, 73, 0.14); }
.tf-chip-miss { color: #d29922; background: rgba(210, 153, 34, 0.14); }
.tf-chip-fab { color: #f0883e; background: rgba(240, 136, 62, 0.14); }
...
<body class="logs-page">
    ...
    <div id="logs-toolbar" class="logs-page-toolbar" hidden>
        <input id="logs-filter" type="text" placeholder="Filter tool calls: agent, tool, path&hellip;" autocomplete="off">
        <label class="logs-failures" title="Only show calls that errored, failed the operation, or used a suspicious path">
            <input id="logs-failures" type="checkbox"> failures &amp; suspicious only
        </label>
    </div>

    <main class="logs-page-main">
        <pre id="logs-body" class="logs-page-body" aria-live="polite"></pre>
        <div id="tools-body" class="tools-feed" aria-live="polite" hidden></div>
    </main>
...
|   |-- dialog_box.py
|   |   |-- dialog_box_logic
|   |   |-- dialog_box_problem_statement
|   |   `-- dialog_box_ui_manifest
|   |-- console_log.py
|   |-- paths.py
|   |-- server.py
|   `-- tool_log.py
...
## role
You are the **Module Builder Agent** (Step 3). Your role is to take the Stage 2 Technical Blueprint directly from the user's message and compile it into a single, complete, production-ready Python custom module file.
...
2. `write_text_file`: Writes text files directly to disk.
3. `map_files`: Inspects directory structures and file trees on disk.
...
## input_contract
Accepts the **Stage 2 Technical Blueprint** passed directly as text in the message.

## workflow
Compile the incoming Stage 2 blueprint into a single Python script file organized into three explicit code phases:
1. **Phase 1 (UI Phase)**: Write top-level `UI_MANIFEST` dictionary.
2. **Phase 2 (Logic & Endpoints)**: Write `register_routes(app)` with real-time `print(f"[<module_id>] ...")` console tracing and `server.paths` imports.
3. **Phase 3 (Variable Map & Hooks)**: Add a variable/object map docstring and extension hooks (`_extension_pre_process_hook`, `_extension_post_process_hook`).

## boundaries
- **Code Only**: Output ONLY the Python script inside a single markdown code block. No preambles, save questions, or text chatter.
- **Complete Code**: No `TODO` or incomplete placeholders in core logic.
- **Console Visibility**: Include `print()` logs prefixed with `[<module_id>]` for real-time terminal tracing.
...
# ==============================================================================
# PHASE 2: LOGIC & SERVER ENDPOINTS PHASE (register_routes + Terminal Logging)
# ==============================================================================
def register_routes(app: FastAPI):
    ...
...
## skills/ux_module_designer_skills.md
